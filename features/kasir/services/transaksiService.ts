...
export class TransaksiService {
  ...
  constructor(
    ...
    // TASK 2: Initialize performance monitor (disabled in production by default)
  ...
  async createTransaksiSizeAware(data: CreateTransaksiRequest): Promise<TransaksiWithDetails> {
    ...
    const queryTimer = this.performanceMonitor.startQuery('createTransaksiSizeAware', {
      itemCount: data.items.length,
      hasDiscount: !!data.discountType,
    })
    ...
    try {
      ...
      const transactionResult = await this.prisma.$transaction(
        async (tx) => {
          ...
          // TASK 2: Start stock check timing
      ...
      // TASK 2: End performance tracking and log results
      this.performanceMonitor.endQuery(queryTimer)

      // Log slow transactions for debugging
      if (this.performanceMonitor.isEnabled()) {
        const metrics = timer.getMetrics()
        if (metrics.totalTime > 3000) {
          console.warn('[Performance] Slow transaction detected:', timer.getSummary())
        }
      }
    }
  }
   ...
   * OPTIMIZED: Uses stockValidationService for bulk validation to eliminate N+1 queries
  ...
  private async validateStockAvailabilityInTransaction(
    ...
    // ✅ VALIDATION 2: Use date-aware bulk availability checking if dates are provided
