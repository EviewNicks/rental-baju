        ...
        async (tx) => {
          try {
            ...
            for (const item of request.items) {
              ...
              // Helper function to calculate penalty per condition with proper distribution
              //eslint-disable-next-line @typescript-eslint/no-explicit-any
              ...
              for (const condition of item.conditions) {
                // Get total penalty for this item
                const totalItemPenalty =
                  penaltyCalculation.itemPenalties.find((p) => p.itemId.startsWith(item.itemId))
                    ?.totalPenalty || 0

                // Calculate individual condition penalty with proper distribution
                const conditionPenalty = calculateConditionPenalty(condition, totalItemPenalty, item.conditions)

                // Enhanced logging for penalty calculation debugging
                kasirLogger.penaltyCalc.info('processUnifiedReturn', 'Condition penalty calculated', {
                  ...
                  totalItemPenalty,
                  nonBaikConditionsCount: item.conditions.filter(c => c.conditionCategory !== 'BAIK').length,
                  distributionMethod: item.conditions.some(c => c.conditionCategory !== 'BAIK' && c.manualPrice > 0) ? 'proportional' : 'equal'
                })
