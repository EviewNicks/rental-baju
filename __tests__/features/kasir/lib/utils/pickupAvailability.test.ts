  ...
  test('Property 7.1: Transaction completion status should be based on all items being fully picked up', () => {
    fc.assert(
      fc.property(transactionArb, (transaction) => {
        ...
        const allItemsFullyPickedUp = transaction.items.every(
          (item) => item.alreadyPickedUp >= item.totalQuantity,
        )

        const hasRemainingItems = transaction.items.some((item) => item.remainingQuantity > 0)
